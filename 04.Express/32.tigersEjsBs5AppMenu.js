const pm = require("path"); //패스모듈 head.js불러오기위한
...
app.get('/', (req, res) => {
   ...
   dm.getList(rows => {
      ejs.renderFile('views/32.indexMenu.ejs', {
...
app.get('/create', (req, res) => {
   ...
   ejs.renderFile('views/32.createMenu.ejs',{
     menu, path
   }, (err, html) => {
      res.send(html);
   });
...
app.get('/update/:id', (req, res) => {  // http://localhost:3000/update?id=123 에서  http://localhost:3000/update/123
   ...
   dm.getPlayer(id, rows => {
      ...
      ejs.renderFile('views/32.updateMenu.ejs', {
...
app.get('/delete/:id', (req, res) => {      // http://localhost/delete?id=123 에서 http://localhost/delete/123로
   const id = parseInt(req.params.id);
   ejs.renderFile('views/32.deleteMenu.ejs', {
      path, id
   }, (err, html) => {
      res.send(html);
   });
