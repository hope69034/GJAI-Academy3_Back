...
const sql = `
SELECT player,backNo,POSITION FROM tigers WHERE POSITION="투수";
`;
...
conn.query(sql, (err,fields)=>{
   if (err)
      throw err;
   sql = `SELECT player,backNo,POSITION FROM tigers WHERE POSITION="투수";`;
   conn.query(sql,(err,rows,fields)=>{
      if (err)
         throw err;
         for (let row of rows){
            console.log(`${row.player}\t${row.backNo}\t${row.POSITION}\t`)
         }
   });
   conn.end();
});
